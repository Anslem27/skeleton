...
void main(List<String> argumentsPre) async {
  ...
  if (argumentsPre.isNotEmpty) {
    ...
    if (argumentsPre.length == 2) {
      stopDatetime = DateTime.tryParse(argumentsPre.last);
    }
  }
  ...
  for (final line in outputLines) {
    ...
    final valTrimmed =
        regexCloseIssue.firstMatch(val)?[0] ??
        regexRefIssue.firstMatch(val)?[0] ??
        val;
  ...
  final stringy = map.entries.map(
    (e) => '- ${e.key}:\n${e.value.reversed.map((e) => '   - $e').join('\n')}',
  );
  final dateText = DateFormat(
    'dd/MM/yyyy',
  ).format(stopDatetime ?? DateTime.now());
  const title = '# Pos Changelog';
  final finalString =
      "$title\n\n## $dateText\n# $versionName\n${stringy.join('\n')}\n${changelog.readAsStringSync().replaceFirst(title, '')}";
  changelog.writeAsStringSync(finalString);

  final stringyFeat = (prettyMap['feat'] ?? []).reversed
      .map((e) => '   • $e')
      .join('\n');
  prettyMap.remove('feat');
  final stringyBugImpr = prettyMap.entries.map(
    (e) =>
        '- ${e.key}:\n${e.value.reversed.map((e) => '   • $e').join('\n')}\n',
  );
